...
namespace HelpersLib
{
    public abstract class SettingsBase<T> where T : SettingsBase<T>, new()
    {
        ...
        public bool IsUpgrade
        {
            get
            {
                return !IsFirstTimeRun && Helper.CompareApplicationVersion(ApplicationVersion) < 0;
            }
        }
        ...
        {
            FilePath = filePath;
            ApplicationVersion = Helper.ProductVersion.ToString();
        ...
        private static bool SaveInternal(object obj, string filePath, bool createBackup)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(filePath))
                {
                    lock (obj)
                    {
                        Helper.CreateDirectoryFromFilePath(filePath);
