...
namespace ShareX.UploadersLib
{
    public static class OAuthManager
    {
        ...
        public static string GenerateQuery(string url, Dictionary<string, string> args, HttpMethod httpMethod, OAuthInfo oauth)
        {
            ...
            string signature = URLHelper.URLEncode(Convert.ToBase64String(signatureData));
        ...
        public static string GetAuthorizationURL(string requestTokenResponse, OAuthInfo oauth, string authorizeURL, string callback = null)
        {
            ...
            if (args[ParameterToken] != null)
            {
                ...
                {
                    url += string.Format("&{0}={1}", ParameterCallback, URLHelper.URLEncode(callback));
                }
        ...
        private static string GenerateSignatureBase(HttpMethod httpMethod, string normalizedUrl, string normalizedParameters)
        {
            ...
            signatureBase.AppendFormat("{0}&", URLHelper.URLEncode(normalizedUrl));
            signatureBase.AppendFormat("{0}", URLHelper.URLEncode(normalizedParameters));
        ...
        {
            return Helper.GetRandomAlphanumeric(12);
        }
        ...
        {
            return string.Join("&", parameters.OrderBy(x => x.Key).ThenBy(x => x.Value).Select(x => x.Key + "=" + URLHelper.URLEncode(x.Value)).ToArray());
        }
    }
}